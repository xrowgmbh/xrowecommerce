...
{/if}
         ...
         <div class="block">
         <div class="element">
<input {if $attribute.data_int|eq('2')} readonly="readonly"{/if} id="ezcoa-{if ne( $attribute_base, 'ContentObjectAttribute' )}{$attribute_base}-{/if}{$attribute.contentclassattribute_id}_{$attribute.contentclass_attribute_identifier}" class="{$html_class} ezcc-{$attribute.object.content_class.identifier} ezcca-{$attribute.object.content_class.identifier}_{$attribute.contentclass_attribute_identifier}" type="text" size="20" name="{$attribute_base}_xrowtin_data_text_{$attribute.id}" value="{$attribute.data_text|wash( xhtml )}" />
</div>
