        ...
        <div class="class-xrow-commerce">
            <div class="xrow-product">
                    ...
                    <div class="xrow-feature-list">
                        ...
                        {if is_set($node.data_map.rating)}
                ...
                <div class="productwrapper float-break">
                    <form name="buy" id="buy" method="post" action={"xrowecommerce/multiadd"|ezurl}>
                        ...
                        <input type="hidden" name="ContentObjectID" value="{$node.object.id}" />
                        <input type="hidden" name="ViewMode" value="full" />
                        {if $node.data_map.options.data_type_string|eq('ezmultioption')}
                            <div class="attribute-multi-options" style="float: left; clear: left;">
                                {foreach $node.object.data_map.options.content.multioption_list as $key => $multiitem}
                                    <p>{$multiitem.name}</p>
                                    <select name="AddToBasketList[0][variations][{$node.data_map.options.id}][{$multiitem.id}]">
                                    {foreach $multiitem.optionlist as $multiattribute}
                                        <option value="{$multiattribute.option_id}">{$multiattribute.value}</option>
                                    {/foreach}
                                    </select><br />
                                {/foreach}
                                <input type="text" name="AddToBasketList[0][quantity]" value="1" />
                                <br />
                                <input type="hidden" name="AddToBasketList[0][object_id]" value="{$node.object.id}" />
                                ...
                                {*$node.object.data_map.options.content.multioption_list|attribute(show)*}
                            </div>
                        {else}
                            <div>
