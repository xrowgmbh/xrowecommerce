<?php
/**
 * File containing the xrowCountryType class.
 *
...
/*!
  \class xrowCountryType xrowCountrytype.php
  \ingroup eZDatatype
...
class xrowCountryType extends eZDataType
{
    const DATA_TYPE_STRING = 'xrowCountry';

    const DEFAULT_LIST_FIELD = 'data_text5';

    const MULTIPLE_CHOICE_FIELD = 'data_int1';

    function xrowCountryType()
    {
        $this->eZDataType( self::DATA_TYPE_STRING, ezpI18n::tr( 'kernel/classes/datatypes', 'xrow Country', 'Datatype name' ),
    ...
    */
    static function fetchTranslatedNames( &$countries )
    {
        ...
        usort( $countries, array( 'xrowCountryType', 'compareCountryNames' ) );
    }
    ...
    */
    static function fetchCountry( $value, $fetchBy = false )
    {
        ...
        $allCountries = xrowCountryType::fetchCountryList();
        $result = false;
    ...
    function fetchClassAttributeHTTPInput( $http, $base, $classAttribute )
    {
        ...
        if ( $http->hasPostVariable( $base . '_xrowCountry_multiple_choice_value_' . $classAttribute->attribute( 'id' ) . '_exists' ) )
        {
             $content['multiple_choice'] = $http->hasPostVariable( $base . "_xrowCountry_ismultiple_value_" . $classAttributeID ) ? 1 : 0;
        }

        if ( $http->hasPostVariable( $base . '_xrowCountry_default_selection_value_' . $classAttribute->attribute( 'id' ) . '_exists' ) )
        {
            if ( $http->hasPostVariable( $base . "_xrowCountry_default_country_list_". $classAttributeID ) )
            {
                $defaultValues = $http->postVariable( $base . "_xrowCountry_default_country_list_". $classAttributeID );
                $defaultList = array();
                foreach ( $defaultValues as $alpha2 )
                {
                    if ( trim( $alpha2 ) == '' )
                        continue;
                    // Fetch xrowCountry by aplha2 code (as reserved in iso-3166 code list)
                    $xrowCountry = xrowCountryType::fetchCountry( $alpha2, 'Alpha2' );
                    if ( $xrowCountry )
                        $defaultList[$alpha2] = $xrowCountry;
                }
    ...
    {
        $content = $classAttribute->content();
        return xrowCountryType::storeClassAttributeContent( $classAttribute, $content );
    }
    ...
    */
    function fetchObjectAttributeHTTPInput( $http, $base, $contentObjectAttribute )
    {
        if ( $http->hasPostVariable( $base . '_country_' . $contentObjectAttribute->attribute( 'id' ) ) )
        {
            ...
            {
                $countries = xrowCountryType::fetchCountryList();
    ...
    */
    function objectAttributeContent( $contentObjectAttribute )
    {
        ...
        {
            $xrowCountry = xrowCountryType::fetchCountry( $alpha2, 'Alpha2' );
            $resultList[$alpha2] = $xrowCountry ? $xrowCountry : '';
        }
...
eZDataType::register( xrowCountryType::DATA_TYPE_STRING, 'xrowCountrytype' );
