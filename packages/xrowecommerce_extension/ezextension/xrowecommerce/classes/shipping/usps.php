...
class USPS extends ShippingInterface
{
    ...
    */
    ...
    /*
     * Get the right shipping method with id from USPS Response
     *
     * // new values
     *  Global Express Guaranteed (GXG) ID 4
     *  Express Mail International ID 1
     */
    function respronseMethodIDs()
    {
        return array( '6' => 1, '7' => 4, 'usps_international' => 1, 'usps_international_guaranteed' => 4 );
    }
    ...
    function loadConfiguration()
    {
        ...
        $this->setAddressFrom( $uspsini->variable( "ShipperSettings", "Country" ), $uspsini->variable( "ShipperSettings", "State" ), $uspsini->variable( "ShipperSettings", "Zip" ), $uspsini->variable( "ShipperSettings", "City" ) );
    }
    ...
    {
        $shipping_country = $this->convert_country( $shipping_country, "Alpha3", "Name" );
        parent::setAddressTo( $shipping_country, $shipping_state, $shipping_zip, $shipping_city );
    }

    /*
     * Get the right shipping method
     */
    function getService( $service_list, $service_name )
    {
        $shippingMethods = $this->respronseMethodIDs();

        foreach ( $service_list as $service )
        {
            if ( $shippingMethods[$service_name] == $service['id'] )
            {
                return $service;
            }
        }
    ...
    function getShippingDetails()
    {
        ...
        ezDebug::writeDebug( $ats, 'USPS XML' );
        ezDebug::writeDebug( $array, 'USPS Response' );

        if ( isset( $array['ERROR'] ) && count( $array['ERROR'] ) )
        { // If it is error
        ...
        elseif ( isset( $array['RATEV2RESPONSE'][0]['PACKAGE'][0]['ERROR'] ) && count( $array['RATEV2RESPONSE'][0]['PACKAGE'][0]['ERROR'] ) )
        {
        ...
        elseif ( isset( $array['INTLRATERESPONSE'][0]['PACKAGE'][0]['ERROR'] ) && count( $array['INTLRATERESPONSE'][0]['PACKAGE'][0]['ERROR'] ) )
        ...
        elseif ( isset( $array['RATEV2RESPONSE'] ) && count( $array['RATEV2RESPONSE'] ) )
        { // if everything OK
            ...
            foreach ( $array['RATEV2RESPONSE'][0]['PACKAGE'][0]['POSTAGE'] as $value )
            {
                ...
                $priceObject->mailservice = $value['MAILSERVICE'][0]['VALUE'];
                $priceObject->rate = $value['RATE'][0]['VALUE'];
                $this->list[] = $priceObject;*/
                
                $price = array();
                $price['mailservice'] = $value['MAILSERVICE'][0]['VALUE'];
                $price['costs'] = $value['RATE'][0]['VALUE'];
                $this->list[] = $price;
            }
        }
        elseif ( isset( $array['INTLRATERESPONSE'][0]['PACKAGE'][0]['SERVICE'] ) && count( $array['INTLRATERESPONSE'][0]['PACKAGE'][0]['SERVICE'] ) )
        { // if it is international shipping and it is OK
            foreach ( $array['INTLRATERESPONSE'][0]['PACKAGE'][0]['SERVICE'] as $value )
            {
                /*$priceObject = new intPrice();
                $priceObject->id = $value['ATTRIBUTES']['ID'];
                $priceObject->pounds = $value['POUNDS'][0]['VALUE'];
                $priceObject->ounces = $value['OUNCES'][0]['VALUE'];
                $priceObject->mailtype = $value['MAILTYPE'][0]['VALUE'];
                $priceObject->country = $value['COUNTRY'][0]['VALUE'];
                $priceObject->rate = $value['POSTAGE'][0]['VALUE'];
                $priceObject->svccommitments = $value['SVCCOMMITMENTS'][0]['VALUE'];
                $priceObject->svcdescription = $value['SVCDESCRIPTION'][0]['VALUE'];
                $priceObject->maxdimensions = $value['MAXDIMENSIONS'][0]['VALUE'];
                $priceObject->maxweight = $value['MAXWEIGHT'][0]['VALUE'];
                $this->list[$priceObject->id] = $priceObject;*/
                $price = array();
                ...
                $price['costs'] = $value['POSTAGE'][0]['VALUE'];
                $price['svccommitments'] = $value['SVCCOMMITMENTS'][0]['VALUE'];
                $price['description'] = $value['SVCDESCRIPTION'][0]['VALUE'];
