...
class XROWRecurringOrderCollection extends eZPersistentObject
{
    ...
    static function createNew( $user_id = null )
    {
        if ( $user_id === null )
            $user_id = eZUser::currentUserID();
        $collection = new XROWRecurringOrderCollection( array( 'user_id' => $user_id, 'status' => XROWRecurringOrderCollection::STATUS_ACTIVE, 'created' => XROWRecurringOrderCollection::now() ) );
        $collection->store();
        return $collection;
    }
