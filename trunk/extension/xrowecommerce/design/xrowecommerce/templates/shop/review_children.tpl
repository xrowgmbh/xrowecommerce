...
<h2>{$children_count} {'Customer Reviews'|i18n('extension/xrowecommerce')}</h2>
...
{if is_unset( $versionview_mode )}
    {if $node.object.can_create}
        ...
        <form method="post" action={"content/action/"|ezurl}>
            <input class="button product-new-review" type="submit" name="NewButton" value="{'Make your own review'|i18n( 'design/ezwebin/full/forum' )}" />
    ...
    {else}
        <p>
        {"You need to be logged in to create reviews. You can do so %login_link_start%here%login_link_end%"|i18n( "extension/xrowecommerce",,
...
{if is_unset( $versionview_mode )}
    {if $node.object.can_create}
        {def $notification_access=fetch( 'user', 'has_access_to', hash( 'module', 'notification', 'function', 'addtonotification' ) )}
        <form method="post" action={"content/action/"|ezurl}>
            <input class="button product-new-review" type="submit" name="NewButton" value="{'Make your own review'|i18n( 'extension/xrowecommerce' )}" />
            <input type="hidden" name="ContentNodeID" value="{$node.node_id}" />
            <input type="hidden" name="ContentObjectID" value="{$node.contentobject_id}" />
            <input type="hidden" name="ContentLanguageCode" value="{ezini( 'RegionalSettings', 'ContentObjectLocale', 'site.ini')}" />
            <input type="hidden" name="NodeID" value="{$node.node_id}" />
